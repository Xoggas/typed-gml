...
public sealed class Verifier(IReadOnlyList<ISemanticCheck> checks, DiagnosticBag diagnostics)
{
    ...
    private void Walk(IAstNode node, VerificationContext ctx, string currentNamespace)
    {
        switch (node)
        {
            ...
            case StaticConstructorDeclarationNode ctor:
                WithMember(FindStaticConstructor(ctx), ctx, true, null, () => { RunChecks(ctor, ctx); Walk(ctor.Body, ctx, currentNamespace); });
                break;
    ...
    private MemberSymbol FindStaticConstructor(VerificationContext ctx) => ctx.CurrentType?.Members.FirstOrDefault(m => m.Kind == MemberKind.StaticConstructor) ?? new MemberSymbol { Name = ".cctor", Kind = MemberKind.StaticConstructor, ReturnType = "void", Modifiers = new HashSet<string>(StringComparer.Ordinal) { "static" } };
